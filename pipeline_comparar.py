...
print(f"\n  MATRIZ DE DISTANCIAS 8x8 - WORST:")
print(f"  {'':>6}", end="")
for v in r_w['variables']:
    print(f" {v:>10}", end="")
print()
...
print("\n  NOTA SOBRE H(Y)=0:")
print("  Y es constante en cada grupo (BEST: Y=1, WORST: Y=0).")
print("  Por eso H(Y)=0 y todas las aristas hacia Y tienen d=0.")
print("  El arbol MIN se vuelve trivial (costo 0).")
print("  Para el analisis de dependencias, usar el arbol MAX (MI directa).")

# Grados MIN y MAX
g_b_min = grados(r_b['prim_min'], 8)
g_w_min = grados(r_w['prim_min'], 8)
g_b_max = grados(r_b['prim_max'], 8)
g_w_max = grados(r_w['prim_max'], 8)
...
def write_sheet(ws, r, fill_color, name):
    ...
    # Matriz IM
    ws.cell(row=row, column=1, value="MATRIZ IM 8x8").font = Font(bold=True, size=12)
    row += 1
    nv = len(r['variables'])
    ...
    # Matriz de distancias
    ws.cell(row=row, column=1, value="MATRIZ DE DISTANCIAS 8x8").font = Font(bold=True, size=12)
    row += 1
    for c, h in enumerate([''] + r['variables'], 1):
        ws.cell(row=row, column=c, value=h)
    style_header(ws, row, nv + 1)
