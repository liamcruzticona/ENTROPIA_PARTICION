...
Una variable es \textbf{crítica} si su $|\Delta|$ se sitúa por encima del
punto de corte determinado por la caída natural (gap detection) en la serie
ordenada, sin depender de umbrales fijos arbitrarios. El gap se calcula como
la razón entre valores consecutivos de $|\Delta|$; el punto de máxima razón
marca la frontera entre variables críticas y estables.
...
inocua}: dos variables ($x_3$ y $x_4$) se sitúan por encima del corte
determinado por caída natural (gap ratio 1.68x), evidenciando que el
dataset original contiene subpoblaciones con estructuras de dependencia
diferenciadas. Las cinco variables restantes muestran robustez estructural.
...
En este enfoque, $x_5$ y $x_2$ emergen como las variables con mayor $|\Delta|$,
mientras que $x_4$ muestra $|\Delta| \approx 0$ (su distancia total a las
demás variables es prácticamente idéntica en ambos grupos).
