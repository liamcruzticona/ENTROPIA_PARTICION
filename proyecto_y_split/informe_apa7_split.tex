...
inocua}: dos de las siete variables ($x_3$ y $x_4$) exceden el rango de
variación permitido, evidenciando que el dataset original contiene
...
\begin{enumerate}
    ...
    \item El método rVP (Rango de Variación Permitido) identificó a
          $x_3$ ($\Delta=-1.685$) y $x_4$ ($\Delta=-0.781$) como las
          únicas variables críticas cuya posición topológica cambia
          significativamente entre BEST y WORST, mientras que $x_1$,
          $x_2$, $x_5$, $x_6$ y $x_9$ son estructuralmente robustas
          ($|\Delta| < 0.5$).
\end{enumerate}
