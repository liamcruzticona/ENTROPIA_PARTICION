...
matriz de distancias, tanto en BEST como en WORST:

\begin{equation*}
    \Sigma_{\text{BEST}}(X_i) = \sum_{j=1}^{7} d_{\text{BEST}}(X_i, X_j)
\end{equation*}
\begin{equation*}
    \Sigma_{\text{WORST}}(X_i) = \sum_{j=1}^{7} d_{\text{WORST}}(X_i, X_j)
\end{equation*}
\begin{equation*}
...
más cambian su distancia total a las demás variables entre BEST y WORST.
...
Se observa que $x_5$, $x_2$, $x_1$ y $x_6$ presentan los mayores $|\Delta|$,
mientras que $x_4$ y $x_9$ tienen $|\Delta| < 0.01$, indicando que su
distancia total a las demas variables practicamente no cambia entre grupos.
Esto es consistente con el hecho de que $x_4$-$x_9$ y $x_6$-$x_9$ mantienen
$d=0$ en ambos grupos.

\subsubsection{Dataset Reducido}

A partir del ranking, se genera un nuevo dataset \texttt{d9\_reducido.csv}
con las 4 variables mas discriminativas ($x_5$, $x_2$, $x_1$, $x_6$),
reduciendo la dimensionalidad de 7 a 4 columnas (42.9\% de reduccion)
...
\begin{enumerate}
    ...
    \item El ranking de variables por $\Delta$ (suma de fila completa
          de la matriz de distancias) identificó a $x_5$, $x_2$, $x_1$
          y $x_6$ como las más discriminativas entre BEST y WORST,
          permitiendo reducir la dimensionalidad del dataset de 7 a 4
          columnas (42.9\% de reducción).
\end{enumerate}
