...
Para cada variable $X_i$ se calcula la suma de las distancias de las
aristas incidentes en el \'arbol MIN (distancias), tanto en BEST como
en WORST:

\begin{equation*}
    \Sigma_{\text{BEST}}(X_i) = \sum_{(u,v) \in \text{MST}_{\text{BEST}}}
    d_{\text{BEST}}(u,v) \cdot \mathbb{1}[u=i \lor v=i]
\end{equation*}
\begin{equation*}
    \Sigma_{\text{WORST}}(X_i) = \sum_{(u,v) \in \text{MST}_{\text{WORST}}}
    d_{\text{WORST}}(u,v) \cdot \mathbb{1}[u=i \lor v=i]
\end{equation*}
\begin{equation*}
...
\begin{table}[H]
    \centering
    \caption{Ranking por $\Delta$ usando \'arbol MIN (distancias)}
    \label{tab:seleccion}
    \small
    ...
    \midrule
    1 & $x_1$  & 1.4497 & 0.4238 & 1.0258 & CONSERVAR \\
    2 & $x_9$  & 0.9885 & 1.9873 & 0.9988 & CONSERVAR \\
    3 & $x_5$  & 1.7854 & 1.7310 & 0.0544 & CONSERVAR \\
    4 & $x_2$  & 0.3439 & 0.3121 & 0.0318 & CONSERVAR \\
    5 & $x_3$  & 0.9967 & 0.9922 & 0.0045 & ELIMINAR \\
    6 & $x_4$  & 0.0000 & 0.0000 & 0.0000 & ELIMINAR \\
    7 & $x_6$  & 0.0000 & 0.0000 & 0.0000 & ELIMINAR \\
    \bottomrule
    \end{tabular}
\end{table}

$x_4$ y $x_6$ tienen $\Delta = 0$ porque mantienen $d=0$ con $x_9$ en ambos
grupos (dependencia perfecta estable). $x_1$ y $x_9$ son las m\'as
discriminativas, cambiando completamente su rol entre BEST y WORST.

\subsubsection{Dataset Reducido}

Se genera \texttt{d9\_reducido.csv} con las 4 variables conservadas
($x_1$, $x_9$, $x_5$, $x_2$), reduciendo la dimensionalidad de 7 a 4
columnas (42.9\% de reducci\'on) sobre las 1000 filas originales.
...
\begin{enumerate}
    ...
    \item El ranking de variables usando el \'arbol MIN (distancias)
          con umbral $|\Delta| \ge 0.01$ identific\'o a $x_1$, $x_9$,
          $x_5$ y $x_2$ como las m\'as discriminativas entre BEST y WORST,
          eliminando a $x_3$, $x_4$ y $x_6$ (42.9\% de reducci\'on).
\end{enumerate}
